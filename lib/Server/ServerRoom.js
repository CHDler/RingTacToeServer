...
class ServerRoom extends colyseus_1.Room {
    constructor() {
        ...
        this.pendingTurnTimerStartToken = 0;
        this.pendingTurnTimerPlayer = -1;
        this.pendingTurnTimerReason = "";
        this.turnTimerAnimationGraceMs = 2000;
        this.clientMoveAnimWaitBeforeRotateMs = 2000;
        this.clientMoveAnimRotatePerStepMs = 1000;
    ...
    scheduleTurnTimerAfterAnimation(reason, animationDelayMs = 0) {
        if (!this.hasStarted)
            return;
    ...
    startPendingTurnTimer(token, source) {
        if (token !== this.pendingTurnTimerStartToken)
            return;
        if (!this.hasStarted)
            return;
        if (this.pendingTurnTimerPlayer !== this.turnPlayer)
            return;
    ...
    estimateClientMoveAnimationMs(move) {
        if (!move)
            return 0;
        const rotations = Array.isArray(move.rotateSteps)
            ? move.rotateSteps
            : [move.rotateStep];
        const maxSteps = rotations.reduce((maxStep, rawStep) => {
    ...
    onCreate(options) {
        ...
        try {
            ...
            this.onMessage("turnAnimationReady", this.safeMessageHandler("turnAnimationReady", (client, data) => this.onTurnAnimationReady(client, data)));
    ...
    onTurnAnimationReady(client, data) {
        const player = this.state.playerStates.get(client.sessionId);
        if (!this.hasStarted || !player)
            return;
        if (player.playerOrder !== this.turnPlayer)
            return;
        if (player.playerOrder !== this.pendingTurnTimerPlayer)
            return;
        const requestedTurnPlayer = Number(data === null || data === void 0 ? void 0 : data.turnPlayer);
    ...
    processMove(player, data, options) {
        ...
        const endGamePayload = this.checkForEnd();
        if (!endGamePayload) {
            this.scheduleTurnTimerAfterAnimation(`${options.actorLabel} move`, this.estimateClientMoveAnimationMs(this.currentMove));
        }
